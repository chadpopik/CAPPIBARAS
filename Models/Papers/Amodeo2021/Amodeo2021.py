...
class Dust():
    ...
    def x(self, nu, T, **kwargs):
        return (nu*c.h/c.k_B/T*u.K).decompose()
    
    def dustpoly(self, pdict={}, **kwargs): # Eq B1, Polynominal dust fit to stacked profiles I(nu) [in jr/sr]
        p = self.p0 | pdict | kwargs
        # Amplitude part
        amp = p['A_dust']*u.kJy/u.sr*(self.nu*(1+p['z0'])/(p['nu0']*u.GHz))**(p['beta_dust']+3)
        # Planck function part
        planck = (np.exp(self.x(self.nu*(1+p['z0']), p['T_dust']*u.K))-1)/(np.exp(self.x(self.nu*(1+p['z0']), p['T_dust']*u.K))-1)
        # Polynomial part
...
class Studies(BaseStudy):  # ui.adsabs.harvard.edu/abs/2021PhRvD.103f3514A
    subs = {}
    info = {
...
class Plots(BasePlots):  # ui.adsabs.harvard.edu/abs/2021PhRvD.103f3514A
